...
export default function BookingFlow() {
  ...
  if (submitted) {
    return (
      <div className="strip-edge-light mx-auto max-w-xl rounded-3xl border border-ink-text/10 bg-paper-raised p-10 text-center shadow-strip">
        <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-flash text-ink">
          <Check size={28} strokeWidth={2.5} />
        </span>
        <h3 className="mt-6 font-display text-2xl font-semibold text-ink-text">
          Got it, {contact.name.split(" ")[0]}!
        </h3>
        <p className="mt-3 text-muted-dark">
          Your inquiry for a {eventType?.toLowerCase()} on{" "}
          <span className="text-ink-text">{contact.date}</span> is in. We'll
          follow up at <span className="text-ink-text">{contact.email}</span>{" "}
          within one business day with availability and a formal quote.
        </p>
        <div className="mt-6 rounded-2xl bg-paper-inset px-6 py-4">
          <p className="text-xs uppercase tracking-wide text-muted-dark">
            Estimated total
          </p>
        ...
        <button
          ...
          className="btn-secondary-light mt-7"
        >
          Start a new inquiry
        </button>
      </div>
  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.4fr_1fr]">
      {/* Wizard */}
      <div className="strip-edge-light rounded-3xl border border-ink-text/10 bg-paper-raised p-6 shadow-strip sm:p-9">
        {/* Progress */}
              ...
              <div className="flex flex-col gap-2 w-full">
                <div className="flex items-center justify-between text-xs">
                  <span
                    className={clsx(
                      "font-medium",
                      i <= step ? "text-ink-text" : "text-muted-dark/60"
                    )}
                  >
                    {label}
                  </span>
                </div>
                <div className="h-1 w-full overflow-hidden rounded-full bg-ink-text/10">
                  <motion.div
          ...
          >
                      ...
                      <button
                        ...
                        className={clsx(
                          ...
                          isActive
                            ...
                            : "border-ink-text/10 bg-paper-inset hover:border-ink-text/25"
                        )}
                      >
                        <span
                          className={clsx(
                            ...
                            isActive
                              ? "bg-flash text-ink"
                              : "bg-paper-inset text-flash-warm"
                          )}
                        >
                          <Icon size={17} />
                        </span>
                        <span>
                          <span className="block font-medium text-ink-text">
                            {type.id}
                          </span>
            ...
            {step === 2 && (
              <div>
                ...
                <div className="mt-6">
                  <label className="mb-2 block text-sm font-medium text-ink-text">
                      ...
                      <button
                        ...
                        className={clsx(
                          ...
                          guestCount === g
                            ...
                            : "border-ink-text/12 text-muted-dark hover:border-ink-text/30"
                        )}
                      >
                        {g}
                      </button>
            ...
            {step === 3 && (
              <div>
                ...
                <div className="mt-7 flex flex-col gap-4">
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-ink-text">
                      Event date
                    </label>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-ink-text/12 bg-paper-inset px-4 py-3 text-sm text-ink-text placeholder:text-muted-dark/60 outline-none transition-colors focus:border-flash"
                    />
                  </div>
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-ink-text">
        ...
        {/* Controls */}
        <div className="mt-9 flex items-center justify-between border-t border-ink-text/10 pt-6">
          <button
            onClick={() => setStep((s) => Math.max(0, s - 1))}
            className={clsx(
              "flex items-center gap-1.5 text-sm font-medium text-muted-dark transition-opacity",
              step === 0 ? "pointer-events-none opacity-0" : "hover:text-ink-text"
            )}
          >
            <ArrowLeft size={15} />
            Back
      ...
      {/* Live summary */}
      <div className="h-fit rounded-3xl border border-ink-text/10 bg-paper-inset p-7 lg:sticky lg:top-28">
        <p className="eyebrow">Your estimate</p>
        <p className="mt-3 font-display text-4xl font-semibold text-flash-soft">
          ${estimate.toLocaleString()}
          <span className="text-base font-normal text-muted-dark"> / event</span>
        </p>

        <div className="mt-6 flex flex-col divide-y divide-ink-text/8">
