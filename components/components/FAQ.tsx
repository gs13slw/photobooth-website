  ...
  return (
    <section id="faq" className="relative py-28">
      <div className="container-x">
        ...
        <div className="mx-auto mt-12 max-w-2xl divide-y divide-ink-text/10 rounded-3xl border border-ink-text/10 bg-paper-raised">
          {FAQS.map((item, i) => {
            ...
            return (
              <div key={item.q} className="px-6 sm:px-8">
                ...
                >
                  <span className="font-medium text-ink-text">{item.q}</span>
                  <span
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-paper-inset text-flash-warm transition-transform duration-300 ${
                      isOpen ? "rotate-45" : ""
                    }`}
                  >
                    <Plus size={15} />
                ...
                <AnimatePresence initial={false}>
                  {isOpen && (
                    ...
                    >
                      <p className="pb-6 text-sm leading-relaxed text-muted-dark">
                        {item.a}
                      </p>
                    </motion.div>
                  )}
